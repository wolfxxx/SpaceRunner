...
class GameScene extends Phaser.Scene{
  ...
  collectPowerup(player,p){
    ...
    if(this.roguelite && this.roguelite.disableDoubleScore && t==='goldStar'){
      ...
      try{ if(this.roguelite.manager && this.roguelite.manager.adjustCurrencies){ this.roguelite.manager.adjustCurrencies({ salvage: 20 }); } }catch(err){}
      Sfx.powerup();
      return;
    }
    if(t==='double'){
  ...
  updateRogueliteBanner(extra){
    if(!this.isRogueliteEnabled()) return;
    this.createRogueliteBanner();
    if(!this.rogueliteBanner) return;
  ...
  applyActiveAbilities(abilities) {
    if(!window.ACTIVE_ABILITIES || !abilities) return;
    ...
    Object.keys(abilities).forEach(abilityKey => {
      ...
      if(keyIndex < 4) {
        ...
        this.input.keyboard.on('keydown-' + keyNum, () => this.activateAbility(abilityKey));
      }
    });
  ...
  createAbilitiesHUD() {
    if(!this.roguelite.activeAbilities) return;
  ...
  activateAbility(abilityKey) {
    if(!this.roguelite.activeAbilities || !this.roguelite.activeAbilities[abilityKey]) return;
    if(!window.ACTIVE_ABILITIES || !window.ACTIVE_ABILITIES[abilityKey]) return;
    ...
    if(now - lastUsed < cooldown) return; // Still on cooldown
  ...
  handleRogueliteLevelComplete(){
    ...
    const salvage=typeof rp.salvage==='number' ? rp.salvage : Math.max(10, Math.round(this.level*15));
    const cores=typeof rp.cores==='number' ? rp.cores : ((this.level%3===0)?1:0);
  ...
  applyTradeOfferModifier(){
    ...
    try{ this.roguelite.manager.adjustCurrencies({ salvage: 40 }); }catch(e){}
    if(typeof this.lives === 'number'){
  ...
  hitBossEscort(bullet, escort){
    ...
    if(escort.hp<=0){
      ...
      try{ this.roguelite.manager.adjustCurrencies({ salvage: 15 }); }catch(e){}
    }
  }
...
// ---------- Start Scene ----------
class StartScene extends Phaser.Scene{ constructor(){ super('StartScene'); } create(){ this._startingGame=false; const w=this.scale.width,h=this.scale.height; const t={fontFamily:'monospace', color:'#fff'}; this.add.text(w/2,h/2-120,'SPACE INVADERS',{...t,fontSize:'52px',color:'#0ff'}).setOrigin(0.5); this.add.text(w/2,h/2-70,'Modern Phaser Edition',{...t,fontSize:'18px',color:'#ccc'}).setOrigin(0.5); const best=(()=>{ try{ return parseInt(localStorage.getItem('si_highscore')||'0',10)||0; }catch(e){ return 0; } })(); this.add.text(w/2,h/2-30,'Best (local): '+best,{...t,fontSize:'18px',color:'#bbb'}).setOrigin(0.5); const lbTitle=this.add.text(w/2, h/2+70, 'Global Top 10', {...t,fontSize:'18px',color:'#0ff'}).setOrigin(0.5);
...
const lbText=this.add.text(listX, lbTitle.y+26, 'Loading leaderboard...', {...t,fontSize:'14px',color:'#bbb'}).setOrigin(0,0);
 ...
 try{
  ...
  try{
    ...
    const rogText=this.add.text(w - 16, 110, `Roguelite: ${rogInit? 'ON':'OFF'} (click)`, {...t,fontSize:'13px',color:'#0ff'})
      .setOrigin(1,0.5).setInteractive({useHandCursor:true});
    rogText.on('pointerdown', (p,lx,ly,ev)=>{ if(ev&&ev.stopPropagation) ev.stopPropagation(); try{ const next=!window.ENABLE_ROGUELITE; if(window.SET_ROGUELITE_MODE) window.SET_ROGUELITE_MODE(next); else window.ENABLE_ROGUELITE=next; rogText.setText(`Roguelite: ${next? 'ON':'OFF'} (click)`); toast(`Roguelite ${next? 'enabled':'disabled'}.`); }catch(e){} });
  }catch(e){}
     const getMetaState = () => {
      try { return (window.RunManager && typeof window.RunManager.getMeta==='function') ? window.RunManager.getMeta() : null; } catch(err) { return null; }
    };
    const currencyText=this.add.text(16,160,'',{...t,fontSize:'14px',color:'#8cf8ff'}).setOrigin(0,0);
    let hangarMenuOpen=false;
    const updateCurrencyPanel=()=>{
      const metaState=getMetaState()||{};
      const salvage=metaState.salvage||0;
      const cores=metaState.cores||0;
      const loadout=metaState.pendingLoadout||{};
      const picks=Object.keys(loadout).filter(key=>loadout[key]);
      const loadoutLine=picks.length?('Loadout: '+picks.join(', ')):'Loadout: none';
      currencyText.setText(`Salvage: ${salvage}    Cores: ${cores}
${loadoutLine}`);
    };
    updateCurrencyPanel();
    const hangarHint=this.add.text(16,currencyText.y+34,'Press H to manage hangar upgrades',{...t,fontSize:'14px',color:'#00ffaa'}).setOrigin(0,0).setInteractive({useHandCursor:true});
    const openHangarMenu=()=>{
      if(hangarMenuOpen) return;
      hangarMenuOpen=true;
      let layer; let escHandler;
      const closeMenu=()=>{
        if(!hangarMenuOpen) return;
        hangarMenuOpen=false;
        if(escHandler) this.input.keyboard.off('keydown-ESC', escHandler);
        try{ layer.destroy(); }catch(e){}
        updateCurrencyPanel();
      };
      layer=this.add.container(0,0).setDepth(3600);
      const dim=this.add.rectangle(400,300,800,600,0x00080f,0.82).setInteractive({useHandCursor:false});
      layer.add(dim);
      const title=this.add.text(400,150,'Hangar Upgrades',{fontFamily:'monospace',fontSize:'26px',color:'#8cf8ff'}).setOrigin(0.5);
      layer.add(title);
      const upgrades=[
        { key:'extraLife', label:'Reinforced Hull (+1 life)', cost:60, description:'Adds an extra life at the start of each run.' },
        { key:'pierceBoost', label:'Pierce Capacitor (+35% charge)', cost:45, description:'Piercing shot charge fills faster during bosses.' },
        { key:'shieldPrep', label:'Shield Cache (start with shield)', cost:40, description:'Spawn with an active shield every node.' },
      ];
      const optionRows=[];
      const salvageLabel=this.add.text(400,200,'',{fontFamily:'monospace',fontSize:'16px',color:'#ffffff'}).setOrigin(0.5);
      layer.add(salvageLabel);
      const refresh=()=>{
        const metaNow=getMetaState()||{};
        const salvageNow=metaNow.salvage||0;
        const coresNow=metaNow.cores||0;
        salvageLabel.setText(`Salvage: ${salvageNow}    Cores: ${coresNow}`);
        const loadoutNow=metaNow.pendingLoadout||{};
        optionRows.forEach(entry=>{
          const active=!!loadoutNow[entry.upgrade.key];
          entry.bg.setStrokeStyle(1, active?0x00ffaa:0x335566, active?1:0.35);
          entry.label.setColor(active?'#00ffaa':'#ffffff');
        });
      };
      const toggleUpgrade=(upgrade)=>{
        const metaNow=getMetaState()||{};
        const salvageNow=metaNow.salvage||0;
        const loadout={ ...(metaNow.pendingLoadout||{}) };
        const active=!!loadout[upgrade.key];
        if(active){
          loadout[upgrade.key]=false;
          try{ window.RunManager.adjustCurrencies({ salvage: upgrade.cost }); }catch(e){}
        } else {
          if(salvageNow<upgrade.cost){ if(typeof toast==='function') toast('Not enough Salvage for '+upgrade.label); return; }
          loadout[upgrade.key]=true;
          try{ window.RunManager.adjustCurrencies({ salvage: -upgrade.cost }); }catch(e){}
        }
        const hasAny=Object.keys(loadout).some(k=>loadout[k]);
        try{ window.RunManager.setPendingLoadout(hasAny?loadout:null); }catch(e){}
        refresh();
        updateCurrencyPanel();
      };
      upgrades.forEach((upgrade,idx)=>{
        const panel=this.add.container(400,260+idx*80);
        const bg=this.add.rectangle(0,0,540,66,0x0b1626,0.92).setInteractive({useHandCursor:true});
        panel.add(bg);
        const label=this.add.text(-240,-18,upgrade.label,{fontFamily:'monospace',fontSize:'16px',color:'#ffffff'}).setOrigin(0,0);
        panel.add(label);
        const desc=this.add.text(-240,6,upgrade.description,{fontFamily:'monospace',fontSize:'12px',color:'#8cf8ff',wordWrap:{width:500}}).setOrigin(0,0);
        panel.add(desc);
        const costText=this.add.text(240,-18,`${upgrade.cost} Salvage`,{fontFamily:'monospace',fontSize:'14px',color:'#ffd36b'}).setOrigin(1,0);
        panel.add(costText);
        bg.on('pointerdown',()=>toggleUpgrade(upgrade));
        panel.setDepth(1);
        layer.add(panel);
        optionRows.push({ upgrade, bg, label });
      });
      const closeBtn=this.add.text(400,520,'Close (Esc)',{fontFamily:'monospace',fontSize:'16px',color:'#00ffaa'}).setOrigin(0.5).setInteractive({useHandCursor:true});
      closeBtn.on('pointerdown',closeMenu);
      layer.add(closeBtn);
      escHandler=()=>closeMenu();
      this.input.keyboard.on('keydown-ESC', escHandler);
      dim.on('pointerdown',closeMenu);
      refresh();
    };
    // Only show old hangar menu if new system is not available
    if(!(window.ENABLE_ROGUELITE && window.HangarScene)) {
      hangarHint.on('pointerdown',openHangarMenu);
      this.input.keyboard.on('keydown-H',(event)=>{ if(event && event.repeat) return; openHangarMenu(); });
    } else {
      hangarHint.setVisible(false); // Hide old hangar hint when new system is available
    }
this.add.text(w/2,h/2+10,'Press SPACE or TAP to start',{...t,fontSize:'18px'}).setOrigin(0.5); 

    // Hangar button (if roguelite enabled)
    if(window.ENABLE_ROGUELITE && window.HangarScene) {
      this.add.text(w/2,h/2+35,'Press H for Hangar Bay',{...t,fontSize:'16px',color:'#00ffaa'}).setOrigin(0.5).setInteractive({useHandCursor:true})
        .on('pointerdown', () => this.scene.start('HangarScene'))
        .on('pointerover', function() { this.setColor('#00ffff'); })
        .on('pointerout', function() { this.setColor('#00ffaa'); });
    ...
    this.add.text(w/2,h/2+60,'Controls: <- -> move, Space fire, P pause, M mute',{...t,fontSize:'16px',color:'#bbb'}).setOrigin(0.5); 
    const launchGame=()=>{ if(this._startingGame) return; this._startingGame=true;
      if(window.ENABLE_ROGUELITE && window.RunManager){
        ...
        }catch(err){ console.warn('[Roguelite] beginRun failed:', err); }
      }
